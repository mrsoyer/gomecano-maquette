...
import { useRecommendedServices } from '@/composables/useServices'
import { useHomeTestimonials, useHomeStats } from '@/composables/useHomeContent'
...
// Fetch recommended services and testimonials from Supabase via RPC
const { data: recommendedServices } = useRecommendedServices(6)
const { data: testimonials } = useHomeTestimonials(6)

// Filter testimonials for B2C clients (computed for reactivity)
const clientTestimonials = computed(() => {
  if (!testimonials.value) return []
  // Filter client testimonials (type: 'client') and limit to 3
  return testimonials.value
    .filter(t => t.type === 'client' || t.type === 'b2c')
    .slice(0, 3)
})
...
// Stats from Supabase (home_get_stats RPC)
const { data: stats } = useHomeStats()

// DEBUG: Log data sources (testimonials déjà déclaré ligne 106)
watchEffect(() => {
  console.log('📊 STATS SOURCE:', stats.value ? 'SUPABASE ✅' : 'Loading...')
  console.log('📊 STATS DATA:', stats.value)
  console.log('📊 TESTIMONIALS COUNT:', testimonials.value?.length || 0)
})
      ...
      <Container>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <!-- Title (using authorRole as service context) -->
            <h3 class="font-bold text-gray-900 mb-2">{{ testimonial.authorRole || 'Service mécanique' }}</h3>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-gray-100">
              <div class="relative">
                <img
                  :src="testimonial.authorAvatar || avatarUrls[clientTestimonials.indexOf(testimonial)]"
              ...
              <div class="flex-1">
                <div class="font-bold text-gray-900">{{ testimonial.authorName }}</div>
